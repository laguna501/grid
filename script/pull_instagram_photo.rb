  ...
  until count < 20
    ...
    medias.each do |media|
      ...
      account.photos.create(
        identifier: media.id,
        full: Photo.save_file(account.username, media.images.low_resolution.url, account.social_type, "full"),
        thumbnail: Photo.save_file(account.username, media.images.standard_resolution.url, account.social_type, "thumbnail")
      )
    end
    count = medias.count
  end
